    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px] max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto pr-2 mt-4">
          <div className="relative pl-6 flex flex-col-reverse gap-y-6 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-muted-foreground/20">
            {/* Lead Entry (Initial Start) at the BOTTOM */}
            {project?.lead?.createdAt && (
              <div className="relative group">
                <div className="absolute -left-[23px] top-1.5 w-4 h-4 rounded-full border-2 border-background bg-amber-500 z-10" />
                <div className="flex flex-col gap-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-medium text-muted-foreground tabular-nums">
                      {format(
                        new Date(project.lead.createdAt),
                        "dd MMM yyyy, HH:mm",
                      )}
                    </span>
                  </div>
            ...
            {project?.startDate && (
              <div className="relative group">
                <div className="absolute -left-[23px] top-1.5 w-4 h-4 rounded-full border-2 border-background bg-green-500 z-10" />
                <div className="flex flex-col gap-1">
                  <div className="flex items-center justify-between">
              ...
              [...project.history]
                ...
                .map((entry: any, i: number) => (
                  <div key={entry.id} className="relative group">
                    {/* Timeline Dot */}
                    <div className="absolute -left-[23px] top-1.5 w-4 h-4 rounded-full border-2 border-background bg-muted-foreground group-last:bg-primary z-10" />

                    <div className="flex flex-col gap-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-foreground">
                          {entry.division.replace(/_/g, " ")}
                        </span>
                        <span className="text-[10px] font-medium text-muted-foreground tabular-nums">
                          {format(
