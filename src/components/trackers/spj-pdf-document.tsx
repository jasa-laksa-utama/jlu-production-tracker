...
export function SPJPDFDocument({ spj, project }: SPJPDFDocumentProps) {
  const itemsList = spj?.items || [];
  const totalRowsCount = Math.max(8, itemsList.length);

  const formatIndonesianDate = (dateVal?: Date | string | null) => {
    if (!dateVal) return "";
  ...
  const formattedDate = formatIndonesianDate(spj?.createdAt || spj?.date) || formatIndonesianDate(new Date());
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerContainer}>
          ...
          <View style={styles.titleBadge}>
            <Text>SURAT PERMINTAAN JASA</Text>
          </View>
        </View>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.tableHeader}>
            ...
            <View style={styles.colMaterial}>
              <Text style={styles.thText}>DESKRIPSI JASA / PEKERJAAN</Text>
            </View>
          ...
          {itemsList.map((it, idx) => {
            ...
            return (
              ...
              >
                ...
                <View style={styles.colMaterial}>
                  <Text style={styles.tdTextBold}>{it.name}</Text>
                </View>
                <View style={styles.colQty}>
          ...
          {Array.from({ length: Math.max(0, 8 - itemsList.length) }).map(
            (_, i) => {
              const idx = itemsList.length + i;
              const isLast = idx === totalRowsCount - 1;
              return (
                <View
                  key={`empty-${i}`}
          ...
          <View style={styles.signatureBox}>
            ...
            <View style={styles.digitalSignContainer}>
              ...
              <Text style={styles.digitalSignDate}>
                {spj.createdAt
                  ? formatIndonesianDate(new Date(spj.createdAt))
                  : formattedDate}
              </Text>
            </View>
          ...
          <View style={styles.signatureBox}>
            ...
            {spj.approvedByPpic ? (
              <View style={styles.digitalSignContainer}>
                <Text style={styles.digitalSignText}>DIGITALLY SIGNED BY</Text>
                <Text style={styles.digitalSignName}>
                  {spj.mengetahuiName || "PPIC"}
                </Text>
                <Text style={styles.digitalSignDate}>
                  {spj.approvedByPpicAt
                    ? formatIndonesianDate(new Date(spj.approvedByPpicAt))
                    : formattedDate}
                </Text>
              </View>
            ) : (
          ...
          <View style={styles.signatureBox}>
            ...
            {spj.approvedByPm ? (
              <View style={styles.digitalSignContainer}>
                <Text style={styles.digitalSignText}>DIGITALLY SIGNED BY</Text>
                <Text style={styles.digitalSignName}>
                  {spj.menyetujuiName || "PROJECT MANAGER"}
                </Text>
                <Text style={styles.digitalSignDate}>
                  {spj.approvedByPmAt
                    ? formatIndonesianDate(new Date(spj.approvedByPmAt))
                    : formattedDate}
                </Text>
              </View>
            ) : (
