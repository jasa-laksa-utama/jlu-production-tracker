...
export function MechanicalProgressTable({ units }: { units: any[] }) {
  ...
  const handleDrop = (unit: any, toIndex: number) => {
    ...
    const items = [...unit.mechanicalItems];
    ...
    startTransition(async () => {
      const res = await reorderMechanicalItems(unit.id, orderedIds);
      if (res.success) {
        toast.success(
  ...
  const handleDetailChange = (
    ...
    startTransition(async () => {
      const res = await updateMechanicalItemDetails(itemId, details);
      if (res.success) {
  ...
  const handleAddItemsSubmit = async () => {
    ...
    try {
      const res = await addMechanicalItemsToUnit(addModalUnit.id, payload);
      if (res.success) {
        toast.success(
          `${payload.length} komponen mekanikal berhasil ditambahkan!`,
        );
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      const res = await deleteMechanicalItem(id);
      if (res.success) {
  ...
  return (
    <div className="space-y-6">
                    ...
                    <div className="rounded-xl border border-border/40 overflow-hidden">
                      {totalItems > 0 ? (
                        <Table>
                          <TableHeader className="bg-muted/40">
                            <TableRow className="hover:bg-transparent">
                              ...
                              <TableHead className="text-center font-semibold text-xs py-3 w-22.5">
                                Procurement (40%)
                              </TableHead>
                              <TableHead className="text-center font-semibold text-xs py-3 w-22.5">
                                PO (10%)
                              ...
                              <TableHead className="text-center font-semibold text-xs py-3 w-25">
                                Progress (%)
                              </TableHead>
                              <TableHead className="text-center font-semibold text-xs py-3 w-12.5">
                                Action
                              </TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {unit.mechanicalItems.map(
                              (item: any, itemIdx: number) => (
                                <TableRow
                                  ...
                                  className={cn(
                                    "hover:bg-muted/30 transition-all select-none text-xs",
                                ...
                                >
                                  ...
                                  {[
                                    ...
                                    const stageMapName: Record<string, string> =
                                      {
                                        procurementQty: "PROCUREMENT",
                                        poQty: "PO",
                                        fabricationQty: "FABRICATION",
                          ...
                          {(() => {
                            ...
                            const totalUnitQty = mItems.reduce(
                              (acc: number, item: any) =>
                                acc + Math.max(1, Number(item.qty || 1)),
                              0,
                            );

                            const sumProcurement = mItems.reduce(
                              (acc: number, item: any) =>
                                acc +
                                Number(
                                  item.procurementQty ??
                                    (item.procurementDone ? item.qty || 1 : 0),
                                ),
                              0,
                            );
                            const sumPo = mItems.reduce(
                              (acc: number, item: any) =>
                                acc +
                                Number(
                                  item.poQty ??
                            ...
                            const sumFabrication = mItems.reduce(
                              (acc: number, item: any) =>
                                acc +
                                Number(
                                  item.fabricationQty ??
                                    (item.fabricationDone ? item.qty || 1 : 0),
                                ),
                              0,
                            );
                            const sumPackaging = mItems.reduce(
                              (acc: number, item: any) =>
                                acc +
                                Number(
                                  item.packagingQty ??
                            ...
                            const procurementPct =
                              totalUnitQty > 0
                                ? (sumProcurement / totalUnitQty) * 40
                                : 0;
                            const poPct =
                              totalUnitQty > 0
                                ? (sumPo / totalUnitQty) * 10
                                : 0;
                            const fabricationPct =
                              totalUnitQty > 0
                                ? (sumFabrication / totalUnitQty) * 45
                                : 0;
                            const packagingPct =
                              totalUnitQty > 0
                                ? (sumPackaging / totalUnitQty) * 5
                                : 0;
                              ...
                              <TableFooter className="bg-muted/30 border-t-2 border-border/70">
                                ...
                                <TableRow className="hover:bg-transparent font-bold text-xs">
                                  ...
                                  <TableCell
                                    className={cn(
                                      "text-center font-semibold text-sm py-2.5",
                                      sumPo >= totalUnitQty && totalUnitQty > 0
                                        ? "text-emerald-600 dark:text-emerald-400 font-semibold"
                                        : "text-foreground",
                                    )}
                                  >
                                ...
                                <TableRow className="hover:bg-transparent font-semibold text-sm bg-muted/50 border-t border-border/40">
                                  ...
                                  <TableCell
                                    className={cn(
                                      "text-center text-sm font-semibold py-2.5",
                                      procurementPct >= 40 && totalUnitQty > 0
                                        ? "text-emerald-600 dark:text-emerald-400 font-semibold"
                                        : "text-foreground",
                                    )}
                                  >
                                    {procurementPct.toFixed(2)}%
                                  </TableCell>
                                  <TableCell
                                    className={cn(
                                      "text-center text-sm font-semibold py-2.5",
                                      poPct >= 10 && totalUnitQty > 0
                                        ? "text-emerald-600 dark:text-emerald-400 font-semibold"
                                        : "text-foreground",
                                    )}
                                  >
                                  ...
                                  <TableCell
                                    className={cn(
                                      "text-center text-sm font-semibold py-2.5",
                                      fabricationPct >= 45 && totalUnitQty > 0
                                        ? "text-emerald-600 dark:text-emerald-400 font-semibold"
                                        : "text-foreground",
                                    )}
                                  >
                                    {fabricationPct.toFixed(2)}%
                                  </TableCell>
                                  <TableCell
                                    className={cn(
                                      "text-center text-sm font-semibold py-2.5",
                                      packagingPct >= 5 && totalUnitQty > 0
                                        ? "text-emerald-600 dark:text-emerald-400 font-semibold"
                                        : "text-foreground",
                                    )}
                                  >
                                  ...
                                  <TableCell
                                    className={cn(
                                      "text-right font-semibold text-sm py-2.5",
                        ...
                        <div className="p-6 text-center text-xs text-muted-foreground">
                          Belum ada komponen mekanikal untuk unit{" "}
        ...
        <DialogContent className="sm:max-w-200! rounded-2xl border-border/80">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-primary font-bold text-base">
              ...
              Tambah Komponen Mekanikal
            </DialogTitle>
            <DialogDescription className="text-xs">
              Tambahkan komponen mekanikal ke{" "}
              <strong>{addModalUnit?.name}</strong> lengkap dengan jumlah dan
              satuan.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 py-2 max-h-87.5 overflow-y-auto pr-1">
            ...
            <div className="flex items-center gap-2 px-1 text-xs font-bold text-muted-foreground">
              <span className="flex-1">Nama Komponen Penggerak</span>
            ...
            {itemsList.map((row, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <Input
                  type="text"
                  placeholder={`Komponen ${idx + 1} (mis: Drive Pulley)`}
                  value={row.name}
      ...
      {/* Confirmation Dialog for Deleting Mechanical Item */}
      <Dialog
